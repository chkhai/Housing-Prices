...
```
Housing-Prices/
├── model_experiment.ipynb       -- მთავარი სამუშაო ფაილი სადაც გამოვცადე სხვადასხვა ალგორითმები 
├── model_inference.ipynb        -- ფაილი, რომელშიც დავაგენერირე submission.csv
└── README.md               
```
---
...
Ridge და Lasso შემთხვევაში შედარებით დიდ ალფებზე ცუდი შედეგი მომცა, ყველაზე კარგი val_rmse იყო როცა ალფა იყო რაც შეიძლება პატარა, ნულთან ძალიან ახლოს და ფაქტობრივად ქონვერჯდება Linear Regressionში.

Tree-based მოდელებზე ვცადე დამატებითი feature-ებიც (50,60), მაგრამ შედეგებზე დიდად გავლენა არ ჰქონია.

XGBoost-მა პირველ ექსპერიმენტზე train_rmse=0.007 val_rmse=0.145 დამიბრუნა, აკეთებდა overfittings. დავამატე პარამეტრები და feature-ების რაოდენობა გავზარდე 50-მდე, ყველაზე კარგი შედეგი მომცა ამ დროს (n_estimators=200, max_depth=3, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8). train_rmse = 0.085 val_rmse = 0.132. იყო ოვერფიტი და ბევრად უფრო კომპლექსურ პატერნებს სწავლობდა ტრეინ დატაში.

Random Forest მოდელებმა სხვადასხვა ჰიპერპარამეტრებით (n_estimators და max_depth) სტაბილური, მაგრამ შედარებით მაღალი error აჩვენეს (~0.144–0.148), იყო ოვერფიტი რადგან gap ტრეინსა და ვალიდაციის mse-ს შორის დიდი იყო. საუკეთესო შედეგი მივიღე RF_n100_depthNone-ით, თუმცა ძალიან დიდი განსხვავება სხვა ვარიანტებთან შედარებით არ ყოფილა.

|სხვადასხვა ალგორითმების საუკეთესო სქორები      | train_rmse | val_rmse | 
|--------------------------|---|---|
| Linear                   | 0.127  | 0.129  |
| RF_n100_depthNone        | 0.0525 | 0.144  |
| XGBoost                  | 0.087  | 0.133  |
| lasso_alpha0.0001        | 0.127  | 0.130  |
| Ridge_alpha0.01          | 0.127  | 0.129  |

საბოლოოდ model-inference ფაილში submission-ს დასაგენირებლად ავირჩიე Linear Regression, რადგან ყველაზე კარგი შედეგები მომცა(tarin_rmse, val_rmse). მოდელი ავტვირთე model registry-ში და იქიდან ჩამოვტვირთე.

<img width="686" height="62" alt="image" src="https://github.com/user-attachments/assets/428b8858-d55b-49f1-b6ae-04d727cadd04" />

## MLFLOW ექსპერიმენტები
ექსპერიმენტები დალოგილია დაგსჰაბზე. repo link - https://dagshub.com/lchkh23/Housing-Prices
თითოეულ run-ში დაილოგა: იმპუტაცისს პარამეტრები, მოდელის ჰიპერპარამეტრი, threshold-ები, ცვლადების რაოდენობები, Train / Val / Test მეტრიკები


## გამოცდილება
ახლიდან რომ ვწერდე ალბათ სხვადასხვა მოდელებისთვის სხვადასხვა ნოუთბუქ ფაილებს გავაკეთებდი რომ feature engineering/selection უფრო მომერგო კონკრეტული მოდელებისთვის. და უფრო ორგანიზებულიც იქნებოდა:). model_experiment-ის ბოლოს რამდენიმე დაკომენტარებული cell არის რომლებიც ტრენინგისთვის გამოვიყენე და ბარემ დავტოვე. შემდეგ დავალებაში ალბათ ასე ვიზამ.
